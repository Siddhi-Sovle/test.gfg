...
class Solution {
    public static ArrayList<Integer> constructList(int q, int[][] queries) {
        ...
        int cumulativeXor = 0; // To keep track of the cumulative XOR value
        ...
        for (int i = 0; i < q; i++) {
            ...
            if (type == 0) {
                list.add(x ^ cumulativeXor); // Apply cumulative XOR to the new element
            } else if (type == 1) {
                cumulativeXor ^= x; // Update the cumulative XOR value
            }
        }
